...
class Procesar extends conexion
{
    ...
    public function listar_articulos($id_empresa)
    {
        ...
        $sql = "SELECT a.art_id as id, a.art_codigo AS codigo , a.art_nombre AS nombre, a.art_descripcion AS descripcion ,art_stock AS stock, art_valor AS valor
        from tbl_articulos a ,tbl_empresas e WHERE e.emp_id=a.idempresa AND  idempresa=:id_empresa";
    ...
    {
        $sql  = "INSERT INTO `tbl_ventas`( `ven_numero`, `ven_fecha`, `ven_subtotal`, `ven_iva`, `ven_total`, `idcliente`, `idempresa`,`idsucursal`,`ven_numero_emision`, `estado`, `xml`) VALUES(:numero,:fecha,:subtotal,:iva,:total,:cliente,:empresa,:idsucursal,:numeroEmision,:estado,:xml)";
    ...
    public function registrar_detalle($datos)
    {
        $sql = "INSERT INTO tbl_detalle_ventas(detven_cantidad,detven_precio,detven_total,idventa,idarticulo,idempresa)
                VALUES(:cantidad,:precio,:total,:venta,:articulo,:idempresa)";
    ...
    {
        $sql = "INSERT INTO tbl_mascotas(mas_codigo, mas_nombre, mas_sexo, mas_fecha, mas_color, mas_color_secundario, mas_esterilizado, mas_tipo, mas_imagen, mas_codigo_qr, idraza, idtenedor, idempresa,chip)
                VALUES(:codigo,:nombre,:sexo,:fecha,:color,:color_secundario,:esterilizado,:tipo,
                :imagen,:qr,:raza,:tenedor,:empresa,:chip)";
    ...
    {
        $sql = "SELECT art_stock AS cantidad FROM tbl_articulos WHERE art_id = :articulo
    ...
    public function EstadoChip ($codigo,$estado,$idempresa,$idsucursal)
    {
        $sql="UPDATE chips SET estado =:estado where numero=:codigo AND  idempresa=:idempresa AND idsucursal=:idsucursal";
        $stmt=$this->db->prepare($sql);
        $stmt->execute([
            "codigo" => $codigo,
            "estado"  => $estado,
