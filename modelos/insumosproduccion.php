...
class insumosproduccion extends conexion
{
    ...
    public function VerificarRegistro($maqv_datos)
    {
        try {
            ...
            $maqv_sql = "INSERT INTO `insumoproduccion`( `articulo`, `cantidad`, `fecha`, `precioUni`, `precioTot`, `idempresa`, `tipo`) VALUES (?,?,?,?,?,?,?)";
            $maqv_stmt = $this->maqv_dbh->prepare($maqv_sql);
    ...
    public function Editar($maqv_datos)
    {
        try {
            ...
            $maqv_sql = "UPDATE insumoproduccion SET articulo=?,cantidad=?,fecha=?,precioUni=?,precioTot=?,idempresa=?,tipo=? WHERE codigo=?";
            $maqv_stmt = $this->maqv_dbh->prepare($maqv_sql);
