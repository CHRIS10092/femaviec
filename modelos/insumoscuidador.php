...
class insumoscuidador extends conexion
{
    ...
    {
        $maqv_sql = "SELECT * FROM insumocuidado";
    ...
    public function VerificarRegistro($maqv_datos)
    {
        try {
            ...
            $maqv_sql = "INSERT INTO `insumoproduccion`( `articulo`, `cantidad`, `fecha`, `precioUni`, `precioTot`, `idempresa`) VALUES (?,?,?,?,?,?)";
            $maqv_stmt = $this->maqv_dbh->prepare($maqv_sql);
    ...
    private function VerificarGalpon($maqv_datos)
    {
        ...
        $maqv_sql = "SELECT articulo FROM insumoproduccion WHERE articulo=? ";
        $maqv_stmt = $this->maqv_dbh->prepare($maqv_sql);
        $maqv_stmt->bindParam(1, $maqv_datos[0]);
        $maqv_stmt->execute();
    ...
    public function Listar()
    {
        ...
        $maqv_stmt->execute();
        while ($maqv_row = $maqv_stmt->fetch()) {
            $data = $maqv_row->codigo . '||' . $maqv_row->producto . '||' . $maqv_row->tipo . '||' . $maqv_row->cantidad  . '||' . $maqv_row->fecha . '||' . $maqv_row->precioUni . '||' . $maqv_row->precioTot . '||' . $maqv_row->idempresa;
            echo '<tr>';
            echo '<td>' . $maqv_row->codigo . '</td>';
            echo '<td>' . $maqv_row->producto . '</td>';
    ...
    public function Editar($maqv_datos)
    {
        try {
            ...
            $maqv_sql = "UPDATE insumoproduccion SET articulo=?,cantidad=?,fecha=?,precioUni=?,precioTot=?,idempresa=? WHERE codigo=?";
            $maqv_stmt = $this->maqv_dbh->prepare($maqv_sql);
            ...
            $maqv_stmt->bindParam(7, $maqv_datos[0]);
            $maqv_stmt->execute();
            echo 1;
        } catch (PDOException $e) {
            echo $e->getMessage();
    ...
    public function Eliminar($maqv_id)
    {
        try {
            ...
            $maqv_sql = "DELETE FROM insumoproduccion WHERE codigo=?";
            $maqv_stmt = $this->maqv_dbh->prepare($maqv_sql);
